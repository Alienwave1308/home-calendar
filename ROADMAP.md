...
### Фаза A4. Редизайн Mini App (выполнено)
...
### Фаза A5. Клиентский booking flow + экспорт календаря (выполнено)
...
### Фаза A6. Мастер-календарь и статусы записей (выполнено)
...
### Фаза A7. Финальная очистка legacy + релиз (выполнено)
...
- Добавлены:
  - `scripts/sql/reset_prod_for_first_master.sql`
  - `docs/PROD_TG_RELEASE.md`
- CI E2E-спеки и отчеты очищены от legacy-названия `Home Calendar` -> `TG Mini App`.
